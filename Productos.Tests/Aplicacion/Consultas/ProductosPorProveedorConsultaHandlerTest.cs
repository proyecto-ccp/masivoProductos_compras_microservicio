...
namespace Productos.Tests.Aplicacion.Consultas
{
    public class ProductosPorProveedorConsultaHandlerTest
    {
        ...
        public async Task Handle_ValidaRespuestas(Resultado res, string msj, HttpStatusCode status)
        {
            ...
            {
                _mockProductoRepositorio.Setup(m => m.ObtenerPorProveedor(It.IsAny<Guid>())).ReturnsAsync(productos);
            }
            else if (res == Resultado.Error)
            {
                _mockProductoRepositorio.Setup(m => m.ObtenerPorProveedor(It.IsAny<Guid>())).ThrowsAsync(new Exception(msj));
            }


            var objPrueba = new ProductosPorProveedorConsultaHandler(_servicio, _mapper);
